...
const HeaderWalletConnect = () => {
  ...
  useEffect(() => {
    async function chechMetamask() {
      ...
      if (isMetaMaskInstalled()) {
        ...
        const { address } = (await getCurrentWalletConnected()) as any
        setWallet(address)
  ...
  const onClickConnect = async () => {
    try {
      ...
      // Wwhen user rejects the request
      setDisabled(false)
      console.error(error)
      return {
        address: '',
  ...
  const addressFormatter = (account: any) => {
    return (
      'Connected: ' +
